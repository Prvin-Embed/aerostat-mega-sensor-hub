/*
 * ================================================================
 * ESP32 — TELEMETRY HUB V2.2 (PRODUCTION GRADE)
 * ================================================================
 * Receives  : Mega CSV string via UART1 (GPIO26=TX, GPIO27=RX)
 * Sensors   : BNO055 (I2C) | DHT22 (GPIO25) | RS485 Anemometer (UART2)
 * Logs to   : SD Card (SPI, CS=GPIO13) — 22-field CSV
 ...
 * RS485_Wind_kmh, RS485_WindDir_deg
 * ================================================================
 */
...
const long    MEGA_TIMEOUT_MS  = 6000; 
...
void setup() {
  Serial.begin(115200);
  Serial.println(F("=== ESP32 Telemetry Hub V2.2 (PROD) ==="));
  ...
  Serial.println(F("[OK] Modbus RTU Active on UART2 (GPIO17/GPIO16)"));
...
void loop() {
  ...
  if (now - previousMillis >= SEND_INTERVAL) {
    ...
    // --- Poll Modbus RS485 Anemometer ---
    float rs485_wind_kmh = -99.0;
    float rs485_wind_deg = -99.0;
    
    uint8_t mb_result = node.readHoldingRegisters(0x0000, 2);
    if (mb_result == node.ku8MBSuccess) {
      uint16_t raw_speed = node.getResponseBuffer(0x00);
      uint16_t raw_dir = node.getResponseBuffer(0x01);
      
      float wind_speed_mps = raw_speed / 100.0;
      rs485_wind_kmh = wind_speed_mps * 3.6;
      rs485_wind_deg = raw_dir;
    } else {
      Serial.print(F("[WARN] Modbus Anemometer offline. Err Code: 0x"));
      Serial.println(mb_result, HEX);
    }

    // --- Assemble Master 22-Field Payload ---
    char finalCSV[512];
    snprintf(finalCSV, sizeof(finalCSV),
      ...
      yaw,        pitch,          roll,
      dht_temp,   dht_hum,
      rs485_wind_kmh, rs485_wind_deg // Added Modbus variables
    );
